...
var AddMediaDialog = {
    init: function () {
        ...
        form.Id.value = tinyMCE.activeEditor.getParam('addmedia_ContentId');
            ...
            try {
                ...
                if (result && result.url) {
                    if (window.parent && window.parent.AddMediaDialog) {
                        window.parent.AddMediaDialog.insertMedia(result);
                    } else {
                        AddMediaDialog.insertMedia(result);
                    }
    ...
    insertMedia: function (result) {
        if (!result && !result.url) return;

        var markup = /\.(jpe?g|png|gif)$/i.test(result.url)
            ? "<img src=\"" + result.url + "\" />"
